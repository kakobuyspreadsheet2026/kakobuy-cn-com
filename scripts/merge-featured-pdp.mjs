 ...
 * merging them into category-products.json and caching MaisonLooks page meta.
 */
...
/** MaisonLooks API category → category-products.json pool key. */
...
async function cacheFeaturedPageMeta(slugs) {
  ...
  console.log(
    `Featured PDP meta: fetching ${missing.length} of ${slugs.length} slugs from MaisonLooks…`,
  );
  ...
  async function fetchPageMeta(slug, attempt = 1) {
    try {
      ...
      productSizes[slug] = parseMaisonLooksAvailableSizes(html);
    } catch (error) {
      if (attempt < 2) {
        await new Promise((resolve) => setTimeout(resolve, 400));
        return fetchPageMeta(slug, attempt + 1);
